...
class IntReply(loader.Module):
    """Интеллектуальный автоответ через @gigachat_bot"""
    strings = {"name": "IntReply"}

    async def client_ready(self, client, db):
        self.client = client
        self.active_user_id = None  # используем только ID
        self.bot_username = "gigachat_bot"
    ...
    async def watcher(self, m):
        ...
        # Только выбранный пользователь по ID
        if m.sender_id != self.active_user_id:
            return
