...
class IntReply(loader.Module):
    ...
    async def intstart(self, m):
        ...
        await m.edit(f"✅ Автоответчик включён для **{user.first_name}**")
    ...
    async def watcher(self, m):
        ...
        if not target_id or m.sender_id != target_id:
            return  # не выбранный пользователь

        # Сбор текста для пересылки
        text_to_send = m.text or ""
        if not text_to_send:
            return

        # Пересылаем сообщению боту
        try:
            bot_entity = await self.client.get_entity("@gigachat_bot")
            sent_msg = await self.client.send_message(bot_entity, text_to_send)
            # Ждём пока бот напишет ответ (1–2 сек)
            ...
            # Отправляем пользователю реплаем
